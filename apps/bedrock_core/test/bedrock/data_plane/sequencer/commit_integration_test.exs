defmodule Bedrock.DataPlane.Sequencer.CommitIntegrationTest do
  ...
  describe "sequencer and commit proxy integration" do
    test "complete flow: version assignment -> commit notification -> read version update" do
      ...
      assert {:ok, ^initial_version} = Sequencer.next_read_version(sequencer_pid)

      # 2. Assign commit versions (simulate commit proxy getting versions)
      assert {:ok, ^initial_version, commit_v1} = Sequencer.next_commit_version(sequencer_pid)
      assert commit_v1 > initial_version

      assert {:ok, ^commit_v1, commit_v2} = Sequencer.next_commit_version(sequencer_pid)
      ...
      assert :ok = Sequencer.report_successful_commit(sequencer_pid, commit_v1)

      # 5. Verify read version updated
      assert {:ok, ^commit_v1} = Sequencer.next_read_version(sequencer_pid)

      # 6. Report second commit
      assert :ok = Sequencer.report_successful_commit(sequencer_pid, commit_v2)

      # 7. Verify read version updated again
      assert {:ok, ^commit_v2} = Sequencer.next_read_version(sequencer_pid)

      # 8. Get another commit version to verify assignment counter advanced
      assert {:ok, ^commit_v2, next_commit} = Sequencer.next_commit_version(sequencer_pid)
    ...
    test "out-of-order commit notifications handled correctly" do
      ...
      assert {:ok, _, v1} = Sequencer.next_commit_version(sequencer_pid)
      assert {:ok, _, v2} = Sequencer.next_commit_version(sequencer_pid)
      assert {:ok, _, v3} = Sequencer.next_commit_version(sequencer_pid)
      ...
      assert :ok = Sequencer.report_successful_commit(sequencer_pid, v2)
      assert {:ok, ^v2} = Sequencer.next_read_version(sequencer_pid)

      assert :ok = Sequencer.report_successful_commit(sequencer_pid, v3)
      assert {:ok, ^v3} = Sequencer.next_read_version(sequencer_pid)

      # Reporting older version shouldn't decrease read_version
      assert :ok = Sequencer.report_successful_commit(sequencer_pid, v1)
      assert {:ok, ^v3} = Sequencer.next_read_version(sequencer_pid)
    end
    ...
    test "version invariants maintained under concurrent operations" do
      ...
      tasks =
        for _i <- 1..10 do
          Task.async(fn ->
            assert {:ok, read_v, commit_v} = Sequencer.next_commit_version(sequencer_pid)
      ...
      for {_, commit_v} <- results do
        assert :ok = Sequencer.report_successful_commit(sequencer_pid, commit_v)
      end

      # Final read version should be the highest commit version
      max_commit_version = Enum.max(commit_versions)
      assert {:ok, ^max_commit_version} = Sequencer.next_read_version(sequencer_pid)
    end
  end
end
