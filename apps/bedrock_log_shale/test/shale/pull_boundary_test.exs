defmodule Bedrock.DataPlane.Log.Shale.PullBoundaryTest do
  ...
  describe "empty log pull boundary conditions" do
    ...
    test "pull from version 0 with timeout returns error immediately without waiting", %{log: log} do
      ...
      assert {:error, :version_too_new} = Log.pull(log, Version.from_integer(0), timeout_in_ms: 500)
      elapsed = System.monotonic_time(:millisecond) - start_time

      # Should return well before the 500ms timeout - allow up to 200ms for GenServer overhead
      assert elapsed < 200, "Should return immediately, but took #{elapsed}ms (timeout was 500ms)"
    end
