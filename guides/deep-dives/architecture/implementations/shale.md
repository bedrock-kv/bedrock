...
This approach balances memory usage with lookup performance. Pulls arrive in sequential order, so the working set of index entries remains manageable even with millions of stored transactions.

## Transaction Serving

Recovery is the pull interface's only consumer—storage servers stream their shard from the log's Demux instead of pulling the raw WAL. When a pull request arrives, Shale uses the version index to locate the requested data quickly. For sequential pulls, which are the common case, Shale can optimize by reading ahead and caching likely-to-be-requested transactions.
...
- **[Storage](../data-plane/storage.md)**: Streams its shard from Shale's Demux for local updates
