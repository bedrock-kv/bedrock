...
# Create a materializer with its shard assignment; params are stored in the
# worker's manifest and handed to the worker at startup
{:ok, worker_ref} =
  Foreman.new_worker(foreman, "materializer_1", :materializer,
    params: %{"shard_id" => 1}
  )
...
- **From Director**: Receives worker creation requests during recovery
  - **Worker Specification**: `new_worker/4` creates specific worker types on demand; `opts[:params]` travels through the worker manifest to startup (this is how a materializer learns its shard assignment)
