...
Recovery establishes exclusive control by locking the old layout's logs and every advertised materializer with the coordinator's exact `{generation, recovery_id}` grant. Locking does two jobs at once. It fences: a locked service accepts recovery effects only from that pair; a lower generation and a foreign id at the same generation both fail closed. Repeating the same pair is safe even when a retry runs in another process. And it informs: each locked service returns its recovery info — logs report the persisted exclusive `available_after` cursor, their first retained transaction for inspection, and their inclusive endpoint; materializers report their durable version and shard assignment. That information is what lets the later phases reuse survivors instead of discarding them.

Logs and materializers persist an identity-bound lifecycle record through `no_grant`, `locked`, `replay_started`, `replay_complete`, and `running`. Startup requires both that record and a marked manifest, so deleting the fence after adoption cannot silently reopen an unfenced worker. Compatible-binary upgrades may temporarily set the Foreman worker option `recovery_authority_migration: :allow_legacy` while the cluster is quiescent. That option lets Foreman coordinate a one-time adoption of an unmarked legacy directory; direct worker startup never migrates it. Return the option to `:disabled` after every worker is marked. A future, corrupt, missing-after-marking, or identity-mismatched record always fails closed.
