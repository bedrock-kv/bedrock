defmodule Bedrock.DataPlane.Materializer.Basalt.Database do
  @moduledoc false

  alias Bedrock.DataPlane.Materializer.Basalt.Keyspace
  alias Bedrock.DataPlane.Materializer.Basalt.MultiVersionConcurrencyControl, as: MVCC
  ...
  def oldest_durable_version(database), do: PersistentKeyValues.oldest_version(database.pkv)

  @spec last_durable_version(database :: t()) :: Bedrock.version()
  def last_durable_version(database), do: PersistentKeyValues.last_version(database.pkv)
  ...
  def load_keys_into_keyspace(pkv, keyspace) do
    ...
    |> Enum.each(fn keys -> :ok = Keyspace.insert_many(keyspace, keys) end)
  end

  @spec apply_transactions(database :: t(), transactions :: [Transaction.encoded()]) ::
          Bedrock.version()
  ...
  def last_committed_version(database), do: MVCC.newest_version(database.mvcc)
  ...
  def fetch(%{key_range: {min_key, max_key}}, key, _version) when key < min_key or key >= max_key,
    do: {:error, :key_out_of_range}

  @spec fetch(t(), Bedrock.key(), Bedrock.version()) :: Bedrock.value() | :not_found
  def fetch(database, key, version) do
    database.mvcc
    |> MVCC.fetch(key, version)
    |> case do
      {:error, :not_found} ->
        cond do
  ...
  * `:utilization` - the utilization of the database (as a percentage, expressed
  ...
  def info(database, stat), do: PersistentKeyValues.info(database.pkv, stat)

  @doc """
  Ensures that the database is durable up to the latest version.
  """
  @spec ensure_durability_to_latest_version(db :: t()) :: :ok
  def ensure_durability_to_latest_version(db), do: ensure_durability_to_version(db, MVCC.newest_version(db.mvcc))
  ...
  def ensure_durability_within_window(database) do
    ...
    trailing_edge =
      0
      |> max(newest_version_int - database.window_size_in_microseconds)
      |> Version.from_integer()

    if database.mvcc |> MVCC.oldest_version() |> Version.older?(trailing_edge) do
  ...
  def ensure_durability_to_version(db, version) do
    if Version.first?(version) do
      ...
      db.mvcc
      |> MVCC.transaction_at_version(version)
      |> then(fn transaction ->
