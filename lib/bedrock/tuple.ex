defmodule Bedrock.Tuple do
  ...
  @spec pack(nil | tuple() | number() | Bedrock.key()) :: Bedrock.key()
  def pack(unpacked), do: unpacked |> to_iolist() |> :erlang.iolist_to_binary()

  @spec unpack(Bedrock.key()) :: nil | tuple() | number() | Bedrock.key()
  def unpack(packed) do
    {key, <<>>} = unpack_value(packed)
    key
  end
  ...
  defp pack_value(tuple, acc) when is_tuple(tuple),
    do: [@stop_marker | pack_tuple_elements(Tuple.to_list(tuple), [<<@nested_tuple_tag>> | acc])]

  defp pack_value(_, _), do: raise(ArgumentError, "Unsupported data type")

  defp pack_tuple_elements([], acc), do: acc
  defp pack_tuple_elements([element | rest], acc), do: pack_tuple_elements(rest, pack_value(element, acc))
  ...
  defp unpack_binary(<<0x00, rest::binary>>, 0, _original, acc),
    do: {acc |> :lists.reverse() |> :erlang.iolist_to_binary(), rest}

  defp unpack_binary(<<0x00, rest::binary>>, offset, original, acc),
