defmodule Bedrock.ControlPlane.Distributor.DeathHealingTest do
  ...
  describe "healing on materializer death" do
    ...
    test "placeholder restart during healing re-points healed slots at the new placeholder" do
      ...
      # The healed tag's eager re-recruitment is still in flight, so the
      # restart republish skips it: the recruit's real pid may already
      # occupy the slot and must never regress to the placeholder.
      refute_receive {:apply_tsl_delta, %{1 => _}, @epoch}, 200

      # When that recruitment fails, the slot is re-asserted with the
      # restarted placeholder's pid, healing the dead pid it still held.
      send(blocked, :release)
      assert_receive {:apply_tsl_delta, %{1 => ^new_placeholder}, @epoch}, 5_000
    end
  end

  describe "init-time :noproc healing combined with the startup sweep" do
    test "a dead snapshot pid is skipped by the sweep, healed to placeholder coverage, eagerly re-recruited, and republished on restart" do
      attach_telemetry(self())
      test_pid = self()

      # A TSL snapshot naming a materializer pid that died before the
      # distributor started: monitoring it yields an immediate :noproc DOWN.
      dead = spawn(fn -> :ok end)
      ref = Process.monitor(dead)
      assert_receive {:DOWN, ^ref, :process, ^dead, _reason}

      # Re-recruitment fails, so the tag stays placeholder-owned (and not
      # recruitment-pending) when the placeholder is restarted below.
      recruitment = %{
        create_worker_fn: fn _foreman, _worker_id, :materializer, _opts ->
          send(test_pid, :create_worker_called)
          {:error, :no_capacity}
      ...
      {distributor, _director} =
        start_distributor(
          recruitment: recruitment,
          transaction_system_layout: %{shard_materializers: %{1 => dead}}
        )

      placeholder = placeholder_of(distributor)

      # The sweep sees the snapshot pid occupying the slot and skips it...
      assert_receive {:telemetry, [:bedrock, :distributor, :coverage_sweep], %{uncovered: 0}, %{cluster: TestCluster}}

      # ...and the :noproc healing swaps the placeholder into the slot.
      assert_receive {:telemetry, [:bedrock, :distributor, :materializer_down], %{},
                      %{cluster: TestCluster, tag: 1, reason: :noproc}}

      assert_receive {:apply_tsl_delta, %{1 => ^placeholder}, @epoch}, 5_000

      # Eager re-recruitment fires (a crash-class death), failing here into
      # backoff; the failure re-asserts the placeholder for the slot.
      assert_receive :create_worker_called, 5_000

      assert_receive {:telemetry, [:bedrock, :distributor, :recruitment, :failed], %{},
                      %{cluster: TestCluster, tag: 1}},
                     5_000

      assert_receive {:apply_tsl_delta, %{1 => ^placeholder}, @epoch}, 5_000

      # The healed tag is placeholder-covered, so a placeholder restart
      # republishes the restarted placeholder's pid into its slot.
      Process.exit(placeholder, :kill)
      wait_until(fn -> placeholder_of(distributor) != placeholder end)
      new_placeholder = placeholder_of(distributor)
      assert_receive {:apply_tsl_delta, %{1 => ^new_placeholder}, @epoch}, 5_000
