defmodule Bedrock.ControlPlane.Distributor.ReadoptionIntegrationTest do
  ...
  test "a spun-down worker never appears as a re-adoption candidate: identification skips the dead name" do
    tmp_dir = "/tmp/readoption_spindown_#{System.unique_integer([:positive])}"
    File.mkdir_p!(tmp_dir)
    on_exit(fn -> File.rm_rf(tmp_dir) end)

    worker_id = "olivine_spindown_#{System.unique_integer([:positive])}"
    worker_name = :"spundown_olivine_#{System.unique_integer([:positive])}"
    idle_timeout = 60_000

    # A REAL olivine worker carrying BOTH halves of the reconciled params:
    # shard identity (re-adoption) and idle policy (spin-down).
    start_supervised!(
      Map.put(
        Olivine.child_spec(
          otp_name: worker_name,
          ...
          id: worker_id,
          path: tmp_dir,
          params: %{"shard_id" => 1, "idle_timeout" => idle_timeout}
        ),
        :restart,
        :temporary
      )
    ...
    # Pre-spin-down the worker is a perfectly good re-adoption candidate:
    # it exposes its shard assignment through the real info protocol.
    assert {:ok, %{shard_id: 1}} = Materializer.info(worker_pid, [:shard_id])

    # Spin it down: idle expiry exits {:shutdown, :idle} and asks its
    # foreman (this test process) to remove the worker entry, which is what
    # reclaims the manifest and on-disk directory.
    ref = Process.monitor(worker_pid)

    :sys.replace_state(worker_pid, fn state ->
      %{state | last_read_at: System.monotonic_time(:millisecond) - idle_timeout - 1}
    end)

    send(worker_pid, :idle_check)
    assert_receive {:DOWN, ^ref, :process, ^worker_pid, {:shutdown, :idle}}, 5_000
    assert_receive {:"$gen_call", from, {:remove_worker, ^worker_id}}, 5_000
    GenServer.reply(from, :ok)
    refute Process.whereis(worker_name)

    # A new epoch's distributor comes up with the dead worker's id STILL in
    # the director's services map (the stale view a warm recovery can carry).
    test_pid = self()
    ...
    # The sweep publishes placeholder coverage; identification hits the dead
    # (unregistered) name, the exit is caught, and the candidate is counted
    # out cleanly - no upgrade delta ever arrives.
    assert_receive {:apply_tsl_delta, %{1 => ^placeholder}, @epoch}, 5_000
    refute_receive {:apply_tsl_delta, _, _}, 500

    # The tag stays placeholder-covered, the distributor is unharmed, and
    # demand-driven recruitment remains the only path off the placeholder.
    assert Process.alive?(distributor)
    assert %DistributorState{placeholder_tags: tags} = :sys.get_state(distributor)
