defmodule Bedrock.DataPlane.Log.Shale.PushingTest do
  ...
  describe "rollover publishes only after the successor cursor is durable" do
    setup do
      dir = Path.join(System.tmp_dir!(), "pushing_roll_publish_#{System.unique_integer([:positive])}")
      File.mkdir_p!(dir)
  ...
  describe "appends decide emptiness from state, never by reading segments" do
    setup do
      dir = Path.join(System.tmp_dir!(), "pushing_no_read_#{System.unique_integer([:positive])}")
      File.mkdir_p!(dir)
